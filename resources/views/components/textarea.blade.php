...
<div class="form-group textarea-container mb-3" id="container-{{ $uniqueId }}" data-max="{{ $max }}">
    ...
    @if ($editor)
        ...
        <input type="hidden" name="{{ $name }}" id="input-{{ $uniqueId }}" value="{{ $value }}"
            {{ $required ? 'required' : '' }}>
    @else
        <textarea name="{{ $name }}" id="textarea-{{ $uniqueId }}" rows="{{ $rows }}"
            placeholder="{{ $placeholder }}" class="form-control {{ $class }}" {{ $required ? 'required' : '' }}
            {{-- Standard maxlength for character limit --}} maxlength="{{ $max }}" oninput="updateCharCount('{{ $uniqueId }}', this.value)"
            style="resize: none;">{{ $value }}</textarea>
    @endif

    <div class="d-flex justify-content-between mt-1">
        <span id="error-{{ $uniqueId }}" class="text-danger small" style="display: none;">Character limit
            reached!</span>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const quill = new Quill('#editor-{{ $uniqueId }}', {
                ...
                modules: {
                    toolbar: [
                        [{'header': [1, 2, 3, false]}],
                        ['bold', 'italic', 'underline', 'strike'], // strike added
                        [{'color': []}, {'background': []}], // This line adds text color and bg color
                        [{'list': 'ordered'}, {'list': 'bullet'}],
                        ['link', 'image', 'clean'] // optional: link and image added
                    ]
                },
